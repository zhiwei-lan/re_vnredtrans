...
class naverOAuth extends AbstractOAuth
{
    /** @var string $API_CODE_URL The API URL to get the Auth Code. */
    private static $API_CODE_URL = 'https://nid.naver.com/oauth2.0/authorize';

    /** @var string $API_TOKEN_URL The API URL to get the Auth token. */
    private static $API_TOKEN_URL = 'https://nid.naver.com/oauth2.0/token';

    /** @var string $API_USER_INFO_URL The API URL to get the user info. */
    private static $API_USER_INFO_URL = 'https://openapi.naver.com/v1/nid/me';
    ...
    public function __construct(string $token = '')
    {
        ...
        $this->client_id     = env('ShieldOAuthConfig.naver.client_id', $this->config->oauthConfigs['naver']['client_id']);
        $this->client_secret = env('ShieldOAuthConfig.naver.client_secret', $this->config->oauthConfigs['naver']['client_secret']);
    }
    ...
    {
        $naverURL= self::$API_CODE_URL."?response_type=code&client_id={$this->client_id}&redirect_uri={$this->callback_url}&state={$state}";
        return $naverURL;
    }
    ...
    protected function fetchUserInfoWithToken(): object
    {
        ...
        // 不在顶层，需要取 response
        $data->email = $data->response->email ?? null;
        $data->name = $data->response->name ?? null;
        $data->profile_image = $data->response->profile_image ?? null;
        $data->naver_id = $data->response->id ?? null;
        return $data;
    }
    ...
    protected function setColumnsName(string $nameOfProcess, object $userInfo): array
    {
        ...
        if($nameOfProcess === 'newUser'){
            $usersColumnsName = [
                ...
                'active'                                      => 1,
                $this->config->usersColumnsName['naver_id']   => $userInfo->naver_id,
                $this->config->usersColumnsName['avatar']     => $userInfo->profile_image,
            ];
        }
