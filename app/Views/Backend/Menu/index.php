...
<?= $this->section('content') ?>
    <?= $this->include('Backend/Menu/update') ?>
        ...
        <div class="col-lg-5">
            <div class="card card-primary card-outline">
                <div id="nestable-menu" class="card-header">
                    ...
                    <div class="btn-group">
                        <button class="btn btn-primary btn-sm save" data-action="save" title="Save"><i class="fa fa-save"></i><span class="hidden-xs">&nbsp;<?= lang('Haoadmin.global.save') ?></span></button>
                    </div>
                    <div class="btn-group">
            ...
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <div class="float-left">
                        <h5><?= lang('Haoadmin.menu.add') ?></h5>
                    </div>
                </div>
                <div class="card-body">
                    <form action="<?= route_to('admin.menu.create') ?>" method="post" class="form-horizontal">
                        <?= csrf_field() ?>
                        <div class="form-group row">
                            <label class="col-sm-2 col-form-label"><?= lang('Haoadmin.menu.fields.parent') ?></label>
                            <div class="col-sm-10">
                               <select class="form-control parent" name="parent_id" style="width: 100%;">
                                ...
                                <span class="help-block">
                                    <i class="fas fa-exclamation-triangle text-danger"></i>&nbsp;<?= lang('Haoadmin.menu.fields.warning_parent') ?>
                                </span>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <div class="input-group">
                                    ...
                                    <input type="text" name="icon" class="icon-picker form-control <?= session('error.icon') ? 'is-invalid' : '' ?>" value="<?= old('icon') ?>" placeholder="<?= lang('Haoadmin.menu.fields.place_icon') ?>" autocomplete="off">
                        ...
                        <div class="form-group row">
                            <label class="col-sm-2 col-form-label"><?= lang('Haoadmin.permission.title') ?></label>
                            <div class="col-sm-10">
...
$(function () {
    ...
    function menu() {
        $.get("<?= route_to('admin.menu.manage') ?>", function(response) {
            $('.dd').nestable({
                maxDepth: 2,
                json: response.data,
                contentCallback: (item) => {
                    return `<i class="${item.icon}"></i>&nbsp;<strong>${item.title}</strong>&nbsp;&nbsp;&nbsp;<a href="<?= base_url() ?>/${item.route}" class="dd-nodrag">${item.route}</a>&nbsp;&nbsp;${item.active==='0'?'<span class="badge bg-danger"><?= lang('Haoadmin.menu.fields.non_active') ?></span>':''}
    ...
    $(document).on('click', '#btn-edit', function(e) {
        ...
        $.ajax({
            url: `<?= route_to('admin.menu.manage') ?>/${$(this).attr('data-id')}/edit`,
            method: 'GET',
            dataType: 'JSON',
            ...
            editForm.find('select[name="permission"]').val(response.data.permission).change();
    ...
    $(document).on('click', '#btn-update', function(e) {
        ...
        $.ajax({
            url: `<?= route_to('admin.menu.manage') ?>/${ $('#menu_id').val() }/update`,
            method: 'POST',
    ...
    $(document).on('click', '#btn-delete', function(e) {
        ...
        .then((result) => {
            if (result.value) {
                $.ajax({
                    url: `<?= route_to('admin.menu.manage') ?>/${$(this).attr('data-id')}/delete`,
                    method: 'POST',
                }).done((data, textStatus, jqXHR) => {
                    Toast.fire({
                        icon: 'success',
